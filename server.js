...
client.on("guildMemberAdd", async member => {
 ...
 let data = await canva.welcome(member, { link: "https://media.discordapp.net/attachments/886920699980152852/887189839064219648/PicsArt_09-13-09.15.22.png",blur: false }) 
...
fetch(`https://api.affiliateplus.xyz/api/chatbot?message=${encodeURIComponent(message.content)}&botname=${client.user.username}&ownername=nateales`)
    .then(res => res.json())
    .then(data => {
...
client.on("ready", () => {
    ...
    console.log("Delicacy Bot version {1.1} has been started")
});
client.on("message", async message => {
if(!message.guild) return;
...
require('http').createServer((req, res) => res.end('Delicacy Bot is alive!')).listen(3000)

client.on('ready', () => {
    client.user.setActivity(`${default_prefix}help | ${client.guilds.cache.size} servers`, {
      type: "STREAMING",
...
console.log("KeepAlive file has been started")
  ...
  if (message.channel.name == sender.name) {
    ...
    fetch(
      `https://api.affiliateplus.xyz/api/chatbot?message=${encodeURIComponent(
        message.content
      )}&botname=${client.user.username}&ownername=nateales`
    )
      .then(res => res.json())
      .then(data => {
